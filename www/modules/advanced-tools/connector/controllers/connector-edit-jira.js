angular.module('vxl-mobile.connector').controller('connectorEditJiraCtrl',function($scope,$ionicPopup,ConnectorService,$state) {
    ...
    {       
       ...
       //  if ($scope.connector.jira === " " || $scope.connector.jira === undefined) {
       //    $scope.validateMain($scope.connector.jira,"Please Enter URL"); return false; }

       //  if (!($scope.connector.jira).match(urlExp)) {
       //      $scope.validateMain($scope.connector.jira,"Please Enter URL in Proper Formate"); return false;  }
       ...
       $scope.UpdateJiraData();

    };

    $scope.UpdateJiraData = function () {
       ConnectorService.updateJiraDataSource($scope.connector,local_storage_data.id).then(
        function (data)
        {
