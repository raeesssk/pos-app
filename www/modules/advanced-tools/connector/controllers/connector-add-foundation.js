angular.module('vxl-mobile.connector').controller('connectorAddFoundationCtrl',function($scope,$ionicPopup) {
    ...
    {       
        ...
        if ($scope.connector.foundationurl === " " || $scope.connector.foundationurl === undefined) {
          $scope.validateMain($scope.connector.foundationurl,"Please Enter URL"); return false; }

        if (!($scope.connector.foundationurl).match(urlExp)) {
            $scope.validateMain($scope.connector.foundationurl,"Please Enter URL in Proper Formate"); return false;  }
